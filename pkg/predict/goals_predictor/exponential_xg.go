...
func (e *ExponentialXgGoalPredictor) PredictScore(homeTeam, awayTeam, season int32, league string, date time.Time) (float64, float64, error) {
	...
	homeMatches, err := util.GetTeamHomeMatchesSince(homeTeam, time.Now().AddDate(-3, 0, 0))
	if err != nil {
		return -1, -1, err
	}
	...
	awayMatches, err := util.GetTeamAwayMatchesSince(awayTeam, time.Now().AddDate(-3, 0, 0))
	if err != nil {
		return -1, -1, err
	}
