...
var weibullShape = map[domain.League]float64{
...
func (p *WeibullOddsGenerator) Generate1x2Probabilities(homeProjected, awayProjected float64, league domain.League) domain.MatchProbability {
	cache = make(map[alphaArgs]float64)
	homeGoalProb := make(map[int]float64)
	awayGoalProb := make(map[int]float64)
	// homeShape := entity.GetHomexGVariance(season - 1)
	// awayShape := entity.GetAwayxGVariance(season - 1)
