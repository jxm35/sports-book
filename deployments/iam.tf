...
resource "aws_iam_policy" "predictor_lambda_policy" {
  name        = "predictor-lambda-sqs-policy"
  description = "IAM policy for Loader Lambda to access SQS and rds"
...
resource "aws_iam_role_policy_attachment" "predictor_lambda_basic_execution" {
