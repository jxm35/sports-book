...
func plotDistribution(caliMap map[db_model.Match]domain.MatchProbability, yearRange string) {
	...
	hf, err := os.Create(fmt.Sprintf("analysis/calibrations/prediction_distribution_%s.png", yearRange))
	if err != nil {
		panic(err)
	}
	defer hf.Close()
...
func plotBetDistribution(bets []betResult, yearRange string) {
	...
	hf, err := os.Create(fmt.Sprintf("analysis/bets/bet_success_distribution_%s.png", yearRange))
	if err != nil {
		panic(err)
	}
	defer hf.Close()
...
func plotBetWinningsDistribution(bets []betResult, yearRange string) {
	...
	hf, err := os.Create(fmt.Sprintf("analysis/bets/bet_win_distribution_%s.png", yearRange))
	if err != nil {
		panic(err)
	}
	defer hf.Close()
...
func plotBetsPlacedDistribution(bets []betResult, yearRange string) {
	...
	hf, err := os.Create(fmt.Sprintf("analysis/bets/bets_placed_distribution_%s.png", yearRange))
	if err != nil {
		panic(err)
	}
	defer hf.Close()
